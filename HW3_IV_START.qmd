...
```{r}
first_stage <- lm(proposed_turbine_3km ~ log_wind_power + mindistlake + mindistlake_sq + longitude + long_sq + latitude + lat_sq + factor(district_id), data = matched_data)
...
```{r}
second_stage <- lm(change_liberal ~ proposed_turbine_3km_HAT + mindistlake + mindistlake_sq + longitude + long_sq + latitude + lat_sq + factor(district_id), data = matched_data)
...
*Response:* A wind turbine proposal causes a 6% drop in Liberal vote share compared to districts with no turbine proposal. This supports the hypothesis that a backlash does exist on the political party that implemented the Green Energy Act.
...
*Response:* IV estimates the causal effect only for districts where turbine proposals were actually influenced by wind availability. In other words, the -0.064 effect only applies to districts that had turbine proposals determined by wind speed.
...
fit_2sls <- ivreg(change_liberal ~ proposed_turbine_3km + mindistlake + mindistlake_sq + longitude + long_sq + latitude + lat_sq + long_lat + as.factor(district_id) | 
log_wind_power + mindistlake + mindistlake_sq + longitude + long_sq + latitude + lat_sq + long_lat + as.factor(district_id), 
data = matched_data)
